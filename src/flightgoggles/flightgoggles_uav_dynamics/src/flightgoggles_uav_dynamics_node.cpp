...
void Uav_Dynamics::publishState(void){
  ...
  GPSReading.pose.position.x = position_[0] + 0;
  GPSReading.pose.position.y = position_[1] + 0;
  GPSReading.pose.position.z = position_[2] + 0;

  GPSReading.pose.orientation.x = attitude_[0] + 0;
  GPSReading.pose.orientation.y = attitude_[1] + 0;
  GPSReading.pose.orientation.z = attitude_[2] + 0;
  GPSReading.pose.orientation.w = attitude_[3] + 0;
  ...
  VelocityReading.twist.linear.x = velocity_[0] + 0;
  VelocityReading.twist.linear.y = velocity_[1] + 0;
  VelocityReading.twist.linear.z = velocity_[2] + 0;
...
void Uav_Imu::getMeasurement(sensor_msgs::Imu & meas, double * angVel,
  ...
  meas.angular_velocity.x = angVel[0] ;
  meas.linear_acceleration.x = accel[0] ;

  meas.angular_velocity.y = angVel[1] ;
  meas.linear_acceleration.y = accel[1] ;

  meas.angular_velocity.z = angVel[2] ;
  meas.linear_acceleration.z = accel[2] ;

  meas.angular_velocity_covariance[0] = 0;
  meas.linear_acceleration_covariance[0] = 0;
  for (size_t i = 1; i < 8; i++){
    ...
    meas.angular_velocity_covariance[8] = 0;
    meas.linear_acceleration_covariance[8] = 0;
  }
}
